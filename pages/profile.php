...
<div class="container" style="max-width: 800px; padding: 2rem 1rem;">
    ...
    <div class="profile-card" style="background: white; border-radius: 12px; padding: 2rem; box-shadow: 0 4px 6px rgba(0,0,0,0.05); margin-bottom: 2rem;">
        <h2 style="font-size: 1.25rem; font-weight: 600; margin-bottom: 1.5rem; border-bottom: 1px solid #eee; padding-bottom: 0.5rem;">Cover Photo</h2>
        <div style="margin-bottom: 1.5rem;">
            <div id="cover-preview" style="width: 100%; height: 180px; border-radius: 8px; background: <?php echo empty($user['cover_path']) ? 'linear-gradient(135deg, ' . htmlspecialchars($user['color']) . ', #0f172a)' : 'transparent'; ?>; overflow: hidden; position: relative; background-size: cover; background-position: center; border: 1px solid #e2e8f0;">
                <?php if (!empty($user['cover_path'])): ?>
                    <img src="<?php echo htmlspecialchars($user['cover_path']); ?>" alt="Cover" style="width: 100%; height: 100%; object-fit: cover;">
                <?php endif; ?>
            </div>
        </div>
        <div style="display: flex; gap: 1rem; align-items: center;">
            <input type="file" id="cover-input" accept="image/jpeg,image/png,image/gif,image/webp" style="display: none;">
            <button type="button" class="btn btn-primary" onclick="document.getElementById('cover-input').click()">Upload Cover Photo</button>
            <?php if (!empty($user['cover_path'])): ?>
                <button type="button" class="btn btn-danger" onclick="removeCover()">Remove Cover</button>
            <?php endif; ?>
        </div>
        <div style="font-size: 0.85rem; color: #64748b; margin-top: 0.75rem;">Recommended size: 1000x300 pixels. Max size: 5MB.</div>
    </div>
    
    <div class="profile-card" style="background: white; border-radius: 12px; padding: 2rem; box-shadow: 0 4px 6px rgba(0,0,0,0.05); margin-bottom: 2rem;">
        <h2 style="font-size: 1.25rem; font-weight: 600; margin-bottom: 1.5rem; border-bottom: 1px solid #eee; padding-bottom: 0.5rem;">Avatar</h2>
        <div style="display: flex; align-items: center; gap: 1.5rem;">
            <div id="avatar-preview" style="width: 100px; height: 100px; border-radius: 50%; background: <?php echo empty($user['avatar_path']) ? htmlspecialchars($user['color']) : 'transparent'; ?>; display: flex; align-items: center; justify-content: center; font-size: 2.5rem; color: white; overflow: hidden;">
                <?php if (!empty($user['avatar_path'])): ?>
                    <img src="<?php echo htmlspecialchars($user['avatar_path']); ?>" alt="Avatar" style="width: 100%; height: 100%; object-fit: cover;">
                <?php else: ?>
            ...
            <div>
                ...
                <button type="button" class="btn btn-primary" onclick="document.getElementById('avatar-input').click()" style="margin-bottom: 0.5rem;">Upload New Avatar</button>
                <div style="font-size: 0.85rem; color: #64748b;">Allowed formats: JPG, PNG, GIF, WEBP. Max size: 5MB.</div>
                <?php if (!empty($user['avatar_path'])): ?>
                    <button type="button" class="btn btn-danger" onclick="removeAvatar()" style="margin-top: 0.5rem; padding: 0.25rem 0.5rem; font-size: 0.8rem;">Remove Avatar</button>
                <?php endif; ?>
            </div>
        </div>
    </div>
    ...
    <form id="profile-form" class="profile-card" style="background: white; border-radius: 12px; padding: 2rem; box-shadow: 0 4px 6px rgba(0,0,0,0.05); margin-bottom: 2rem;">
        ...
        <div class="form-group" style="margin-bottom: 1.5rem;">
            ...
            <input type="text" id="full_name" name="full_name" required value="<?php echo htmlspecialchars($user['name']); ?>" style="width: 100%; padding: 0.75rem; border: 1px solid #cbd5e1; border-radius: 6px; font-size: 1rem;">
        </div>

        <div class="form-group" style="margin-bottom: 1.5rem;">
            <label style="display: block; margin-bottom: 0.5rem; font-weight: 500; font-size: 0.875rem;">Email Address</label>
            <input type="email" disabled value="<?php echo htmlspecialchars($user['email']); ?>" style="width: 100%; padding: 0.75rem; border: 1px solid #cbd5e1; border-radius: 6px; font-size: 1rem; background: #f1f5f9; color: #64748b;">
            <div style="font-size: 0.8rem; color: #64748b; margin-top: 0.25rem;">Email address cannot be changed.</div>
        </div>

        <div class="form-group" style="margin-bottom: 1.5rem;">
            <label style="display: block; margin-bottom: 0.5rem; font-weight: 500; font-size: 0.875rem;">Phone Number</label>
            <input type="text" id="phone" name="phone" value="<?php echo htmlspecialchars($user['phone'] ?? ''); ?>" style="width: 100%; padding: 0.75rem; border: 1px solid #cbd5e1; border-radius: 6px; font-size: 1rem;">
        </div>

        <div class="form-group" style="margin-bottom: 1.5rem;">
            <label style="display: block; margin-bottom: 0.5rem; font-weight: 500; font-size: 0.875rem;">Work Experience</label>
            <textarea id="work_experience" name="work_experience" rows="3" style="width: 100%; padding: 0.75rem; border: 1px solid #cbd5e1; border-radius: 6px; font-size: 1rem; resize: vertical;"><?php echo htmlspecialchars($user['work_experience'] ?? ''); ?></textarea>
        </div>

        <div class="form-group" style="margin-bottom: 1.5rem;">
            <label style="display: block; margin-bottom: 0.5rem; font-weight: 500; font-size: 0.875rem;">Social Links</label>
            <div id="social-links-container" style="display: flex; flex-direction: column; gap: 0.75rem;">
                <!-- dynamic social links will go here -->
            </div>
            <button type="button" onclick="addSocialLink()" style="margin-top: 0.75rem; background: transparent; border: 1px dashed #cbd5e1; padding: 0.5rem 1rem; border-radius: 6px; cursor: pointer; color: #475569; font-weight: 500; display: inline-flex; align-items: center; gap: 0.5rem;">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 5v14M5 12h14"/></svg> Add Social Link
            </button>
        ...
        <div class="form-group" style="margin-bottom: 1.5rem;">
            ...
            <div style="display: flex; align-items: center; border: 1px solid #cbd5e1; border-radius: 6px; overflow: hidden; max-width: 100%;">
                <div style="background: #f1f5f9; padding: 0.75rem; border-right: 1px solid #cbd5e1; color: #64748b; white-space: nowrap;">/u/</div>
                <input type="text" id="profile_slug" name="profile_slug" value="<?php echo htmlspecialchars($user['profile_slug'] ?? ''); ?>" placeholder="my-custom-name" style="width: 100%; padding: 0.75rem; border: none; font-size: 1rem; outline: none;">
            </div>
            <div id="slug-status" style="font-size: 0.85rem; margin-top: 0.5rem; font-weight: 500;"></div>
        </div>

        <?php if ($publicProfileUrl): ?>
            <div style="background: #f8fafc; padding: 1.5rem; border-radius: 8px; border: 1px solid #e2e8f0; margin-bottom: 1.5rem; text-align: center;">
