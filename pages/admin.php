...
if ($view === 'overview') {
    ...
    $verifyRequests = $db->query("
        SELECT vr.*, u.full_name as user_full_name, u.email as user_email, u.profile_slug 
        FROM verification_requests vr
        JOIN users u ON vr.user_id = u.id
        ORDER BY vr.created_at DESC
    ")->fetchAll();
    $totalPending = (int)$db->query("SELECT COUNT(*) FROM verification_requests WHERE status = 'pending'")->fetchColumn();
    $totalApproved = (int)$db->query("SELECT COUNT(*) FROM verification_requests WHERE status = 'approved'")->fetchColumn();
    $totalRejected = (int)$db->query("SELECT COUNT(*) FROM verification_requests WHERE status = 'rejected'")->fetchColumn();
}
...
<div class="admin-layout" id="admin-panel">
    ...
    <aside class="admin-sidebar" id="admin-sidebar">
        ...
        <nav class="admin-nav">
            ...
            <a href="/admin?view=verify" class="admin-nav-item <?= $view === 'verify' ? 'active' : '' ?>">
                <svg class="admin-nav-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
    ...
    <main class="admin-main-content">
        ...
        <div class="admin-top-bar">
            <h1 class="admin-page-title">
                ...
                elseif ($view === 'profiles')
                    echo 'User Profile Cards';
                elseif ($view === 'unassigned')
                    echo 'Unassigned Folders';
                elseif ($view === 'maintenance')
        ...
        <?php if ($view === 'overview'): ?>
            ...
            <div class="admin-section" style="padding: 1.5rem;">
                <div
                    ...
                    <select id="chart-selector" class="admin-search-input"
                        style="width: auto; padding: 0.4rem 1rem; cursor: pointer; border-radius: 6px; border: 1px solid var(--admin-border);">
                                    ...
                                    <tr>
                                        <td>
                                            <div style="display: flex; align-items: center; gap: 0.75rem;">
                                                <div
                                                    style="width: 32px; height: 32px; border-radius: 6px; background: #f1f5f9; color: var(--admin-text-muted); display: flex; align-items: center; justify-content: center;">
                                                ...
                                                <div>
                                                    <div
                                                        style="font-weight: 500; color: var(--admin-text-main); max-width: 200px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">
                                                        <?= htmlspecialchars($f['display_name']) ?>
                                                    </div>
                                                    <a href="/<?= $f['slug'] ?>" target="_blank"
                                                        style="color: #3b82f6; font-size: 0.75rem; text-decoration: none;">/<?= $f['slug'] ?></a>
                                                </div>
                                            </div>
                                        </td>
                                        <td>
                                            <?php if (!empty($f['full_name'])): ?>
                                                <div style="font-size: 0.875rem; color: var(--admin-text-main); font-weight: 500;">
                                                    <?= htmlspecialchars($f['full_name']) ?></div>
                                                <div style="font-size: 0.75rem; color: var(--admin-text-muted);">
                                                    <?= htmlspecialchars($f['email']) ?></div>
                                            <?php else: ?>
                                        ...
                                        <td>
                                            <div style="font-size: 0.875rem; color: var(--admin-text-main); font-weight: 500;">
                                                <?= formatFileSize($f['total_size'] ?? 0) ?></div>
                                            <div style="font-size: 0.75rem; color: var(--admin-text-muted);">
                                                <?= $f['total_files'] ?> files</div>
                                        </td>
                                        <td>
            ...
            <div class="admin-section">
                <div class="admin-section-header">
                    ...
                    <div class="admin-search-box">
                        <svg class="admin-search-icon" width="18" height="18" viewBox="0 0 24 24" fill="none"
                            stroke="currentColor" stroke-width="2">
                            <circle cx="11" cy="11" r="8" />
                            <line x1="21" y1="21" x2="16.65" y2="16.65" />
                        </svg>
                        <input type="text" id="user-search" class="admin-search-input"
                ...
                <div id="bulk-users-toolbar" class="bulk-actions-toolbar">
                    <span style="font-weight: 600; color: var(--admin-text-main);" id="selected-users-count-text">0 user(s)
                        selected</span>
                    <div style="display: flex; gap: 10px; align-items: center;">
                        <button class="btn btn-sm btn-outline-danger" id="btn-delete-selected-users" disabled
                            style="font-size: 0.8rem; padding: 0.4rem 0.8rem; display: flex; align-items: center; gap: 6px;">
                        ...
                        <button class="btn btn-sm btn-danger" id="btn-delete-all-users"
                            style="font-size: 0.8rem; padding: 0.4rem 0.8rem; display: flex; align-items: center; gap: 6px;">
                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                stroke-width="2">
                                <path
                ...
                <div class="admin-table-container">
                    <table class="admin-table">
                        <thead>
                            <tr>
                                <th style="width: 40px; text-align: center;"><input type="checkbox" id="select-all-users"
                        ...
                        <tbody id="user-table-body">
                            <?php foreach ($users as $u): ?>
                                <tr class="user-row"
                                    data-search="<?= strtolower(htmlspecialchars($u['full_name'] . ' ' . $u['email'])) ?>">
                                    <td style="text-align: center;">
                                        <?php if ($u['id'] !== $currentUser['id']): ?>
                                            <input type="checkbox" class="user-select-chk" value="<?= $u['id'] ?>"
                                                style="cursor:pointer; width: 16px; height: 16px; margin: 0 auto; display: block;">
                                        <?php endif; ?>
                                    </td>
                                    <td>
                                        <div style="display: flex; align-items: center; gap: 1rem;">
                                            <div
                                                style="width: 40px; height: 40px; border-radius: 50%; background: <?= $u['avatar_color'] ?>; color: white; display: flex; align-items: center; justify-content: center; font-weight: 600; font-size: 1.125rem;">
                                    ...
                                    <td>
                                        ...
                                        <div style="font-weight: 600; color: var(--admin-text-main);">
                                            <?= formatFileSize($u['total_used_size']) ?> used</div>
                                        <div style="font-size: 0.875rem; color: var(--admin-text-muted); margin-top: 2px;">Space
                                            Limit: <?= $u['space_limit_mb'] ?> MB • Upload Limit:
                                            <?= $u['file_upload_limit_mb'] ?? 50 ?> MB • <?= $u['folder_count'] ?> /
                                            <?= $u['folder_limit'] ?? 3 ?> folders</div>
                                        <div
                                            style="width: 120px; height: 6px; background: #e2e8f0; border-radius: 4px; margin-top: 6px; overflow: hidden;">
                                            <div
                                                style="height: 100%; width: <?= $usagePct ?>%; background: <?= $usagePct > 90 ? '#ef4444' : '#3b82f6' ?>;">
                                            </div>
                                        </div>
                                    </td>
                                    <td>
                                        <div style="font-size: 0.875rem; color: var(--admin-text-main);">
                                            <span
                                                style="display: block; color: var(--admin-text-muted); font-size: 0.75rem; text-transform: uppercase; margin-bottom: 2px;">Last
                                                Login</span>
                                    ...
                                    <td>
                                        <div style="display: flex; gap: 0.5rem; flex-wrap: wrap;">
                                            <button class="btn btn-sm btn-outline-primary"
                                                ...
                                                onclick="openEditUserModal(<?= $u['id'] ?>, '<?= htmlspecialchars(addslashes($u['full_name'])) ?>', <?= $u['folder_limit'] ?? 3 ?>, <?= $u['space_limit_mb'] ?>, <?= $u['file_upload_limit_mb'] ?? 50 ?>, <?= $u['is_verified'] ?? 0 ?>)">Edit
                                                User</button>
                                            <button class="btn btn-sm btn-outline-secondary"
                                                style="font-size: 0.8rem; padding: 0.4rem 0.8rem;"
                                                onclick="toggleUserFolders(<?= $u['id'] ?>)">Folders</button>
                                            <button class="btn btn-sm btn-outline-danger"
                                                style="font-size: 0.8rem; padding: 0.4rem 0.8rem;"
                                                onclick="deleteAllUserFolders(<?= $u['id'] ?>)">Clear All</button>
                                            <?php if ($u['id'] !== $currentUser['id']): ?>
                                                <button class="btn btn-sm btn-danger"
                                                    style="font-size: 0.8rem; padding: 0.4rem 0.8rem;"
                                                    onclick="deleteUser(<?= $u['id'] ?>)">Delete User</button>
                                            <?php endif; ?>
                                        </div>
                                    </td>
                                </tr>
                                <tr id="user-folders-<?= $u['id'] ?>" style="display: none; background: #f8fafc;">
                                    <td colspan="5" style="padding: 1.5rem; border-bottom: 1px solid var(--admin-border);">
                                        <h4
                                            style="font-weight: 600; margin-bottom: 1rem; color: var(--admin-text-main); font-size: 1rem;">
                                            Folders for <?= htmlspecialchars($u['full_name']) ?></h4>
                                        <?php
                                        ...
                                        if (empty($foldersList)):
                                            ?>
                                            <div
                                                style="padding: 1rem; text-align: center; color: var(--admin-text-muted); background: white; border: 1px dashed var(--admin-border); border-radius: 8px;">
                                                No folders created yet.</div>
                                        <?php else: ?>
                                            <div
                                                style="display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 1rem;">
                                                <?php foreach ($foldersList as $f): ?>
                                                    <div
                                                        style="background: white; border: 1px solid var(--admin-border); border-radius: 8px; padding: 1rem; box-shadow: 0 1px 2px rgba(0,0,0,0.02); display: flex; justify-content: space-between; align-items: flex-start;">
                                                        <div>
                                                            <div
                                                                style="font-weight: 600; color: var(--admin-text-main); margin-bottom: 0.25rem;">
                                                                <?= htmlspecialchars($f['display_name']) ?></div>
                                                            <a href="/<?= $f['slug'] ?>" target="_blank"
                                                                style="color: #3b82f6; font-size: 0.875rem; display: inline-flex; align-items: center; gap: 4px; margin-bottom: 0.5rem; text-decoration: none;">
                                                                /<?= $f['slug'] ?>
                                                            ...
                                                            <div
                                                                style="font-size: 0.75rem; color: var(--admin-text-muted); display: flex; align-items: center; gap: 0.5rem;">
                                                                <span
                                                                    style="background: #f1f5f9; padding: 2px 6px; border-radius: 4px;"><?= formatFileSize($f['total_size']) ?></span>
                                                                <span><?= $f['total_files'] ?> files</span>
                                                            </div>
                                                        </div>
                                                        <button class="btn-icon"
                                                            style="color: #ef4444; border: none; background: #fef2f2; padding: 8px; border-radius: 6px; cursor: pointer; transition: all 0.2s;"
                                                            onclick="deleteFolder(<?= $f['id'] ?>)" title="Delete Folder">
                                                            <svg width="16" height="16" viewBox="0 0 24 24" fill="none"
                                                                stroke="currentColor" stroke-width="2">
                                                                <path
                                                                    d="M3 6h18M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2" />
            ...
            <div class="admin-section">
                <div class="admin-section-header">
                    ...
                    <div class="admin-search-box">
                        ...
                        <input type="text" id="profile-search" class="admin-search-input"
                            placeholder="Search profiles by name or slug...">
                    </div>
                </div>
                ...
                if (empty($profileUsers)):
                    ...
                    <div style="padding: 3rem 1rem; text-align: center;">
                        <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"
                            style="opacity:.3; margin: 0 auto 1rem; color: var(--admin-text-muted);">
                        ...
                        <h4 style="font-weight: 600; color: var(--admin-text-main); margin-bottom: 0.5rem;">No Profile Cards
                            Generated</h4>
                        <p style="color: var(--admin-text-muted); font-size: 0.875rem;">Users have not created any public
                            profile cards yet.</p>
                    </div>
                <?php else: ?>
                    <!-- Bulk Action Toolbar -->
                    <div id="bulk-profiles-toolbar" class="bulk-actions-toolbar">
                        <span style="font-weight: 600; color: var(--admin-text-main);" id="selected-profiles-count-text">0
                            card(s) selected</span>
                        <div style="display: flex; gap: 10px; align-items: center;">
                            <button class="btn btn-sm btn-outline-danger" id="btn-delete-selected-profiles" disabled
                                style="font-size: 0.8rem; padding: 0.4rem 0.8rem; display: flex; align-items: center; gap: 6px;">
                                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                    stroke-width="2">
                                    <path
                            ...
                            <button class="btn btn-sm btn-danger" id="btn-delete-all-profiles"
                                style="font-size: 0.8rem; padding: 0.4rem 0.8rem; display: flex; align-items: center; gap: 6px;">
                                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                    stroke-width="2">
                                    <path
                    ...
                    <div class="admin-table-container">
                        <table class="admin-table">
                            <thead>
                                <tr>
                                    <th style="width: 40px; text-align: center;"><input type="checkbox" id="select-all-profiles"
                                            style="cursor:pointer; width: 16px; height: 16px; margin: 0 auto; display: block;">
                                    </th>
                                    <th>User</th>
                                    <th>Profile Link</th>
                            ...
                            <tbody id="profile-table-body">
                                <?php foreach ($profileUsers as $pu): ?>
                                    <tr class="profile-row"
                                        data-search="<?= strtolower(htmlspecialchars($pu['full_name'] . ' ' . $pu['profile_slug'])) ?>">
                                        <td style="text-align: center;">
                                            <input type="checkbox" class="profile-select-chk" value="<?= $pu['id'] ?>"
                                                style="cursor:pointer; width: 16px; height: 16px; margin: 0 auto; display: block;">
                                        </td>
                                        <td>
                                            <div style="display: flex; align-items: center; gap: 0.75rem;">
                                                <?php if (!empty($pu['avatar_path'])): ?>
                                                    <div style="width: 36px; height: 36px; border-radius: 50%; overflow: hidden;"><img
                                                            src="<?= htmlspecialchars($pu['avatar_path']) ?>" alt=""
                                                            style="width:100%;height:100%;object-fit:cover;"></div>
                                                <?php else: ?>
                                                    <div
                                                        style="width: 36px; height: 36px; border-radius: 50%; background: <?= $pu['avatar_color'] ?>; color: white; display: flex; align-items: center; justify-content: center; font-weight: 600; font-size: 1rem;">
                                                        <?= strtoupper(substr($pu['full_name'], 0, 1)) ?>
                                                    </div>
                                                <?php endif; ?>
                                                <div>
                                                    <div style="font-weight: 600; color: var(--admin-text-main);">
                                                        <?= htmlspecialchars($pu['full_name']) ?></div>
                                                    <div
                                                        style="font-size: 0.75rem; color: var(--admin-text-muted); margin-top: 2px;">
                                                        @<?= htmlspecialchars($pu['profile_slug']) ?></div>
                                                </div>
                                            </div>
                                        </td>
                                        <td>
                                            <a href="/u/<?= htmlspecialchars($pu['profile_slug']) ?>" target="_blank"
                                                style="color: #3b82f6; font-size: 0.875rem; text-decoration: none; display: inline-flex; align-items: center; gap: 4px;">
                                        ...
                                        <td>
                                            <div style="display: flex; gap: 0.5rem; flex-wrap: wrap;">
                                                <a href="/u/<?= htmlspecialchars($pu['profile_slug']) ?>" target="_blank"
                                                    ...
                                                    style="font-size: 0.8rem; padding: 0.4rem 0.8rem;">View</a>
                                                <button class="btn btn-sm btn-outline-danger"
                                                    style="font-size: 0.8rem; padding: 0.4rem 0.8rem;"
                                                    onclick="deleteProfileCard(<?= $pu['id'] ?>)">Delete Card</button>
                                            </div>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
            ...
            <div class="admin-section">
                <div class="admin-section-header">
                    ...
                    <div class="admin-search-box">
                        ...
                        <input type="text" id="folder-search" class="admin-search-input"
                            placeholder="Search folders by name or URL...">
                    </div>
                </div>
                ...
                <?php if (empty($unassignedFolders)): ?>
                    <div style="padding: 3rem 1rem; text-align: center;">
                        ...
                        <h4 style="font-weight: 600; color: var(--admin-text-main); margin-bottom: 0.5rem;">No unassigned
                            folders found</h4>
                        <p style="color: var(--admin-text-muted); font-size: 0.875rem;">All folders currently belong to
                            registered users.</p>
                    </div>
                <?php else: ?>
                    <!-- Bulk Action Toolbar -->
                    <div id="bulk-actions-toolbar" class="bulk-actions-toolbar">
                        <span style="font-weight: 600; color: var(--admin-text-main);" id="selected-count-text">0 folder(s)
                            selected</span>
                        <div style="display: flex; gap: 10px; align-items: center;">
                            <button class="btn btn-sm btn-outline-danger" id="btn-delete-selected" disabled
                                style="font-size: 0.8rem; padding: 0.4rem 0.8rem; display: flex; align-items: center; gap: 6px;">
                                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                    stroke-width="2">
                                    <path
                            ...
                            <button class="btn btn-sm btn-danger" id="btn-delete-all-unassigned"
                                style="font-size: 0.8rem; padding: 0.4rem 0.8rem; display: flex; align-items: center; gap: 6px;">
                                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                    stroke-width="2">
                                    <path
                    ...
                    <div class="admin-table-container">
                        <table class="admin-table">
                            <thead>
                                <tr>
                                    <th style="width: 40px; text-align: center;"><input type="checkbox" id="select-all-folders"
                                            style="cursor:pointer; width: 16px; height: 16px; margin: 0 auto; display: block;">
                                    </th>
                                    <th>Folder Info</th>
                                    <th>Link</th>
                            ...
                            <tbody id="folder-table-body">
                                <?php foreach ($unassignedFolders as $f): ?>
                                    <tr class="folder-row"
                                        data-search="<?= strtolower(htmlspecialchars($f['display_name'] . ' ' . $f['slug'])) ?>">
                                        <td style="text-align: center;"><input type="checkbox" class="folder-select-chk"
                                                value="<?= $f['id'] ?>"
                                                style="cursor:pointer; width: 16px; height: 16px; margin: 0 auto; display: block;">
                                        </td>
                                        <td>
                                            <div style="display: flex; align-items: center; gap: 0.75rem;">
                                                <div
                                                    style="width: 36px; height: 36px; border-radius: 8px; background: #f1f5f9; color: var(--admin-text-muted); display: flex; align-items: center; justify-content: center;">
                                                ...
                                                <div>
                                                    ...
                                                    <div
                                                        style="font-size: 0.75rem; color: var(--admin-text-muted); margin-top: 2px;">
                                                        Created <?= timeAgo($f['created_at']) ?></div>
                                                </div>
                                            </div>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <div style="font-weight: 600; color: var(--admin-text-main);">
                                                <?= formatFileSize($f['total_size']) ?></div>
                                            <div style="font-size: 0.875rem; color: var(--admin-text-muted); margin-top: 2px;">
                                                <?= $f['total_files'] ?> files</div>
                                        </td>
                                        <td>
                                            <button class="btn btn-sm btn-outline-danger"
                                                style="font-size: 0.8rem; padding: 0.4rem 0.8rem;"
                                                onclick="deleteFolder(<?= $f['id'] ?>)">Delete</button>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            </tbody>
        ...
        <?php elseif ($view === 'maintenance'): ?>
            <div class="admin-section" style="padding: 2rem;">
                ...
                <p class="admin-section-subtitle" style="margin-bottom: 2rem;">When maintenance mode is active, only
                    administrators can access the site. Other users will see a maintenance page.</p>

                <div
                    style="background: #f8fafc; border: 1px solid var(--admin-border); padding: 1.5rem; border-radius: 8px; display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 1rem;">
                    <div>
                        <div
                            style="font-weight: 600; color: var(--admin-text-main); font-size: 1.1rem; margin-bottom: 0.25rem;">
                            Maintenance Mode</div>
                        <div style="font-size: 0.875rem; color: var(--admin-text-muted);">Current Status: <strong
                                style="color: <?= $maintenanceMode ? '#ef4444' : '#22c55e' ?>;"><?= $maintenanceMode ? 'Active (Site Offline)' : 'Inactive (Site Online)' ?></strong>
                        </div>
                    </div>
                    <div style="flex-shrink: 0;">
                        <button onclick="toggleMaintenanceMode()"
                            class="admin-btn <?= $maintenanceMode ? 'admin-btn-outline' : 'admin-btn-primary' ?>"
                            style="<?= $maintenanceMode ? 'border-color: #ef4444; color: #ef4444;' : '' ?>">
        ...
        <?php elseif ($view === 'verify'): ?>
            <div class="admin-stats-grid" style="margin-bottom: 1.5rem;">
                <div class="admin-stat-card">
                    <div class="admin-stat-icon-wrapper" style="background: #fef7e0; color: #eab308;">
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                            <circle cx="12" cy="12" r="10" />
                            <polyline points="12 6 12 12 16 14" />
                        </svg>
                    ...
                    <div class="admin-stat-content">
                        <div class="admin-stat-value" id="stat-pending"><?= number_format($totalPending) ?></div>
                        <div class="admin-stat-title">Pending</div>
                    </div>
                </div>
                <div class="admin-stat-card">
                    <div class="admin-stat-icon-wrapper" style="background: #f0fdf4; color: #22c55e;">
                    ...
                    <div class="admin-stat-content">
                        <div class="admin-stat-value" id="stat-approved"><?= number_format($totalApproved) ?></div>
                        <div class="admin-stat-title">Approved</div>
                    </div>
                </div>
                <div class="admin-stat-card">
                    <div class="admin-stat-icon-wrapper" style="background: #fef2f2; color: #ef4444;">
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                            <circle cx="12" cy="12" r="10" />
                    ...
                    <div class="admin-stat-content">
                        <div class="admin-stat-value" id="stat-rejected"><?= number_format($totalRejected) ?></div>
                        <div class="admin-stat-title">Rejected</div>
                    </div>
                </div>
            </div>
            ...
            <div class="admin-section">
                <div class="admin-section-header">
                    <div>
                        <h3 class="admin-section-title">Verification Queries</h3>
                        <p class="admin-section-subtitle">Manage verified badge requests from users</p>
                    </div>
                    <div class="admin-search-box">
                        <svg class="admin-search-icon" width="18" height="18" viewBox="0 0 24 24" fill="none"
                            stroke="currentColor" stroke-width="2">
                        ...
                        <input type="text" id="verify-search" class="admin-search-input"
                            placeholder="Search queries by name or email...">
                    </div>
                </div>

                <?php if (empty($verifyRequests)): ?>
                    <div style="padding: 3rem 1rem; text-align: center;">
                        <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"
                            style="opacity:.3; margin: 0 auto 1rem; color: var(--admin-text-muted);">
                            <path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"></path>
                        ...
                        <h4 style="font-weight: 600; color: var(--admin-text-main); margin-bottom: 0.5rem;">No verification
                            requests</h4>
                        <p style="color: var(--admin-text-muted); font-size: 0.875rem;">There are no applications submitted at
                            this time.</p>
                    </div>
                <?php else: ?>
                    <div class="admin-table-container">
                        <table class="admin-table">
                            ...
                            <tbody>
                                <?php foreach ($verifyRequests as $req): ?>
                                    <tr class="verify-row" data-search="<?= strtolower(htmlspecialchars(($req['user_full_name'] ?? '') . ' ' . ($req['user_email'] ?? '') . ' ' . ($req['real_name'] ?? '') . ' ' . ($req['email'] ?? ''))) ?>">
                                        <td>
                                            <div style="font-weight: 600; color: var(--admin-text-main);">
                                                <?= htmlspecialchars($req['user_full_name']) ?></div>
                                            <div style="font-size: 0.8rem; color: var(--admin-text-muted);">
                                                <?= htmlspecialchars($req['user_email']) ?></div>
                                            <a href="/u/<?= htmlspecialchars($req['profile_slug']) ?>" target="_blank"
                                                style="color: #3b82f6; font-size: 0.75rem; text-decoration: none;">View Profile
                                                &rarr;</a>
                                        </td>
                                        <td>
                                            <div style="font-size: 0.9rem;"><strong>Name:</strong> <span
                                                    id="req-name-<?= $req['id'] ?>"><?= htmlspecialchars($req['real_name']) ?></span>
                                            </div>
                                            <div style="font-size: 0.9rem;"><strong>Phone:</strong> <span
                                                    id="req-phone-<?= $req['id'] ?>"><?= htmlspecialchars($req['phone']) ?></span>
                                            </div>
                                            <div style="font-size: 0.9rem;"><strong>Email:</strong> <span
                                                    id="req-email-<?= $req['id'] ?>"><?= htmlspecialchars($req['email']) ?></span>
                                            </div>
                                        </td>
                                        <td>
                                            <?php if (!empty($req['nid_path'])): ?>
                                                <a href="<?= htmlspecialchars($req['nid_path']) ?>" target="_blank"
                                                    class="admin-btn admin-btn-outline"
                                                    style="padding: 0.35rem 0.7rem; font-size: 0.8rem; display: inline-flex; align-items: center; gap: 4px;">
                                                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                                        stroke-width="2">
                                            ...
                                            <?php else: ?>
                                                <span style="color: var(--admin-text-muted); font-size: 0.85rem;">No file
                                                    uploaded</span>
                                            <?php endif; ?>
                                        </td>
                                        <td>
                                            <?php if ($req['status'] === 'approved'): ?>
                                                <span
                                                    style="background: #e6f4ea; color: #137333; padding: 0.25rem 0.75rem; border-radius: 9999px; font-size: 0.75rem; font-weight: 600; text-transform: uppercase;">Approved</span>
                                            <?php elseif ($req['status'] === 'rejected'): ?>
                                                <span
                                                    style="background: #fce8e6; color: #c5221f; padding: 0.25rem 0.75rem; border-radius: 9999px; font-size: 0.75rem; font-weight: 600; text-transform: uppercase;">Rejected</span>
                                            <?php else: ?>
                                                <span
                                                    style="background: #fef7e0; color: #b06000; padding: 0.25rem 0.75rem; border-radius: 9999px; font-size: 0.75rem; font-weight: 600; text-transform: uppercase;">Pending</span>
                                            <?php endif; ?>
                                        </td>
                                        <td>
                                            <span
                                                style="font-size: 0.85rem; color: var(--admin-text-muted);"><?= timeAgo($req['created_at']) ?></span>
                                        </td>
                                        <td>
                                            <div style="display: flex; flex-direction: column; gap: 0.35rem; width: 90px;">
                                                <button class="admin-btn admin-btn-outline"
                                                    style="padding: 0.35rem 0.7rem; font-size: 0.8rem; background: white; width: 100%; text-align: center; box-sizing: border-box;"
                                                    onclick="openEditVerifyModal(<?= $req['id'] ?>, '<?= htmlspecialchars($req['real_name'], ENT_QUOTES) ?>', '<?= htmlspecialchars($req['phone'], ENT_QUOTES) ?>', '<?= htmlspecialchars($req['email'], ENT_QUOTES) ?>')">
                                                    Edit Info
                                                </button>
                                                <?php if ($req['status'] === 'pending'): ?>
                                                    <button class="admin-btn admin-btn-primary"
                                                        style="padding: 0.35rem 0.7rem; font-size: 0.8rem; background: #16a34a; border-color: #16a34a; width: 100%; text-align: center; box-sizing: border-box;"
                                                        onclick="processVerification(<?= $req['id'] ?>, 'approve')">
                                                        Verify
                                                    </button>
                                                    <button class="admin-btn admin-btn-primary"
                                                        style="padding: 0.35rem 0.7rem; font-size: 0.8rem; background: #dc2626; border-color: #dc2626; width: 100%; text-align: center; box-sizing: border-box;"
                                                        onclick="processVerification(<?= $req['id'] ?>, 'reject')">
                                                        Reject
                                                    </button>
                                                <?php endif; ?>
                                            </div>
...
<!-- Edit Verification Request Modal -->
<div class="modal-backdrop" id="edit-verify-modal"
    style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(15, 23, 42, 0.5); z-index: 1000; align-items: center; justify-content: center; backdrop-filter: blur(2px);">
    <div class="modal-content"
        style="background: white; padding: 2rem; border-radius: 12px; width: 100%; max-width: 400px; box-shadow: 0 20px 25px -5px rgba(0,0,0,0.1);">
        <h3 style="font-size: 1.25rem; font-weight: 600; margin-bottom: 1.5rem; color: var(--admin-text-main);">Edit
            Verification Details</h3>
        <form id="edit-verify-form">
            <input type="hidden" id="edit-verify-id" name="request_id">
            <div class="form-group mb-4">
                <label
                    style="display: block; margin-bottom: 0.5rem; font-weight: 500; color: var(--admin-text-main);">Real
                    Name</label>
                <input type="text" id="edit-verify-name" name="real_name" required
                    style="width: 100%; padding: 0.5rem; border: 1px solid #d1d5db; border-radius: 6px; outline: none;">
            </div>
            <div class="form-group mb-4">
                <label
                    style="display: block; margin-bottom: 0.5rem; font-weight: 500; color: var(--admin-text-main);">Contact
                    Number</label>
                <input type="text" id="edit-verify-phone" name="phone" required
                    style="width: 100%; padding: 0.5rem; border: 1px solid #d1d5db; border-radius: 6px; outline: none;">
            </div>
            <div class="form-group mb-4">
                <label
                    style="display: block; margin-bottom: 0.5rem; font-weight: 500; color: var(--admin-text-main);">Gmail/Email
                    Address</label>
                <input type="email" id="edit-verify-email" name="email" required
                    style="width: 100%; padding: 0.5rem; border: 1px solid #d1d5db; border-radius: 6px; outline: none;">
            </div>
            <div style="display: flex; gap: 1rem; justify-content: flex-end; margin-top: 2rem;">
                <button type="button" class="admin-btn admin-btn-outline"
                    onclick="closeEditVerifyModal()">Cancel</button>
                <button type="submit" class="admin-btn admin-btn-primary" id="btn-save-verify">Save Changes</button>
            </div>
        </form>
    </div>
</div>
...
<div class="modal-backdrop" id="edit-user-modal"
    ...
    <div class="modal-content"
        ...
        <form id="edit-user-form">
            ...
            <div class="form-group mb-4">
                <label
                    style="display: block; margin-bottom: 0.5rem; font-weight: 500; color: var(--admin-text-main);">User
                    Name</label>
                <input type="text" id="edit-user-name" disabled
                    style="width: 100%; padding: 0.5rem; border: 1px solid #d1d5db; border-radius: 6px; background: #f8fafc; color: var(--admin-text-muted);">
            </div>
            <div class="form-group mb-4">
                <label
                    style="display: block; margin-bottom: 0.5rem; font-weight: 500; color: var(--admin-text-main);">Folder
                    Limit</label>
                <input type="number" id="edit-folder-limit" name="folder_limit" min="1" required
                    style="width: 100%; padding: 0.5rem; border: 1px solid #d1d5db; border-radius: 6px; outline: none;">
            </div>
            <div class="form-group mb-4">
                <label
                    style="display: block; margin-bottom: 0.5rem; font-weight: 500; color: var(--admin-text-main);">Space
                    Limit (MB)</label>
                <input type="number" id="edit-space-limit" name="space_limit_mb" min="1" required
                    style="width: 100%; padding: 0.5rem; border: 1px solid #d1d5db; border-radius: 6px; outline: none;">
            </div>
            <div class="form-group mb-4">
                <label
                    style="display: block; margin-bottom: 0.5rem; font-weight: 500; color: var(--admin-text-main);">File
                    Upload Limit (MB)</label>
                <input type="number" id="edit-upload-limit" name="file_upload_limit_mb" min="1" required
                    style="width: 100%; padding: 0.5rem; border: 1px solid #d1d5db; border-radius: 6px; outline: none;">
            </div>
            <div
                style="display: flex !important; flex-direction: row !important; align-items: center !important; justify-content: flex-start !important; gap: 0.5rem !important; margin-top: 1rem; margin-bottom: 1.5rem; width: 100%; text-align: left !important;">
                <input type="checkbox" id="edit-user-verified" name="is_verified" value="1"
                    style="width: 18px !important; height: 18px !important; min-width: 18px !important; cursor: pointer; margin: 0 !important; padding: 0 !important; display: inline-block !important; -webkit-appearance: checkbox !important; appearance: checkbox !important;">
                <label for="edit-user-verified"
                    style="font-weight: 500; color: var(--admin-text-main); cursor: pointer; user-select: none; margin: 0 !important; display: inline-block !important; text-align: left !important; width: auto !important; height: auto !important;">Verified
                    Badge</label>
            </div>
            <div style="display: flex; gap: 1rem; justify-content: flex-end; margin-top: 2rem;">
...
<script>
    ...
    function openEditVerifyModal(requestId, realName, phone, email) {
        document.getElementById('edit-verify-id').value = requestId;
        document.getElementById('edit-verify-name').value = realName;
        document.getElementById('edit-verify-phone').value = phone;
        document.getElementById('edit-verify-email').value = email;
        document.getElementById('edit-verify-modal').style.display = 'flex';
    }

    function closeEditVerifyModal() {
        document.getElementById('edit-verify-modal').style.display = 'none';
    }

    async function processVerification(requestId, action) {
        const actionText = action === 'approve' ? 'verify and approve' : 'reject';
        ...
        customConfirm(
            ...
            async () => {
                ...
                try {
                    const res = await fetch('/api/admin-verify-request', { method: 'POST', body: formData });
    ...
    (function () {
        const editVerifyForm = document.getElementById('edit-verify-form');
        if (editVerifyForm) {
            editVerifyForm.addEventListener('submit', async (e) => {
                e.preventDefault();
                const btn = document.getElementById('btn-save-verify');
                btn.disabled = true;
                btn.textContent = 'Saving...';

                const formData = new FormData(editVerifyForm);
                formData.append('action', 'update_verify_request');
                formData.append('csrf_token', getCSRF());
                ...
                try {
                    ...
                    if (data.success) {
                        showToast('Verification query updated successfully!');
                        setTimeout(() => location.reload(), 1000);
                    } else {
    ...
    function openEditUserModal(userId, name, folderLimit, limit, uploadLimit, isVerified) {
        ...
        document.getElementById('edit-user-verified').checked = (parseInt(isVerified) === 1);
    ...
    (function () {
        ...
        if (userSearch) {
            userSearch.addEventListener('input', (e) => {
                ...
                document.querySelectorAll('.user-row').forEach(row => {
                    ...
                    const textSearch = row.textContent.toLowerCase();
                    if (dataSearch.includes(term) || textSearch.includes(term)) {
        ...
        if (verifySearch) {
            verifySearch.addEventListener('input', (e) => {
                const term = e.target.value.toLowerCase().trim();
